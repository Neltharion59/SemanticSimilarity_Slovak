...
for optimizer_run_name in output_table:
    ...
    for dataset_name in output_table[optimizer_run_name]["raw"]:
        ...
        for dataset_version in output_table[optimizer_run_name]:
            ...
            hyperparameters = output_table[optimizer_run_name][dataset_version][dataset_name_local]['best_model']['hyperparams']
            ...
            for hyperparameter in hyperparameters:
                output_overleaf += "\t\t\t\\item " + hyperparameter + " : " + str(hyperparameters[hyperparameter]) + "\n"
            output_overleaf += "\t\t\\end{itemize}\n"

            if len(string_based) > 0:
                output_overleaf += "\t\t\\item String-based features\n"
                output_overleaf += "\t\t\\begin{itemize}\n"
                for feature in string_based:
                    output_overleaf += "\t\t\t\\item " + feature + "\n"
                    args = input_dict[feature]['args']
                    if len(args) > 0:
                        output_overleaf += "\t\t\t\\begin{itemize}\n"
                        for arg_name in args:
                            if arg_name == 'n_jobs':
                                continue
                            output_overleaf += "\t\t\t\t\\item " + arg_name + " : " + str(args[arg_name]) + "\n"
                        output_overleaf += "\t\t\t\\end{itemize}\n"
                output_overleaf += "\t\t\\end{itemize}\n"

            if len(corpus_based) > 0:
                output_overleaf += "\t\t\\item Corpus-based features\n"
                output_overleaf += "\t\t\\begin{itemize}\n"
                for feature in corpus_based:
                    output_overleaf += "\t\t\t\\item " + feature + "\n"
                    args = input_dict[feature]['args']
                    if len(args) > 0:
                        output_overleaf += "\t\t\t\\begin{itemize}\n"
            ...
            if len(knowledge_based) > 0:
                output_overleaf += "\t\t\\item Knowledge-based features\n"
                output_overleaf += "\t\t\\begin{itemize}\n"
                for feature in knowledge_based:
                    output_overleaf += "\t\t\t\\item " + feature + "\n"
                    args = input_dict[feature]['args']
                    if len(args) > 0:
                        output_overleaf += "\t\t\t\\begin{itemize}\n"
    ...
    output_overleaf = output_overleaf.replace('_', '\\textunderscore ')
